...
common = function ($, code, order, reservation, item, conflicts, keyvalues, image, attachment, inflection, validation, utils, slimdown, kit) {
  /**
   * Return common object with different helper methods
   */
  return $.extend({}, code, order, reservation, item, conflicts, keyvalues, image, attachment, validation, utils, kit);
}(jquery, common_code, common_order, common_reservation, common_item, common_conflicts, common_keyValues, common_image, common_attachment, common_inflection, common_validation, common_utils, common_slimdown, common_kit);
...
helper = function ($, settings) {
  return {
    ...
    getStat: function (stats, type, name, mode) {
      if (!stats || !stats.detailed)
        throw 'Invalid stats object';
...
Reservation = function ($, api, Transaction, Conflict) {
  ...
  Reservation.prototype.canReserve = function () {
    return this.status == 'creating' && this.location && this.contact && this.from && this.to && this.items && this.items.length;
  };
  /**
