...
common_contact = function (Helper, imageHelper) {
  return {
    /**
     * getContactImageUrl 
     *
    ...
    getContactImageUrl: function (ds, contact, size) {
      ...
      if (contact.user && contact.user.picture)
        return new Helper().getImageUrl(ds, contact.user.picture, size);
      // Show avatar initials
      return imageHelper.getAvatarInitial(contact.name, size);
    }
  };
}(helper, common_image);
common = function ($, code, order, reservation, item, conflicts, keyvalues, image, attachment, inflection, validation, utils, slimdown, kit, contact) {
  /**
   * Return common object with different helper methods
   */
  return $.extend({}, code, order, reservation, item, conflicts, keyvalues, image, attachment, validation, utils, kit, contact);
}(jquery, common_code, common_order, common_reservation, common_item, common_conflicts, common_keyValues, common_image, common_attachment, common_inflection, common_validation, common_utils, common_slimdown, common_kit, common_contact);
