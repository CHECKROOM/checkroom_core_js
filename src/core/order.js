			...
			return this._getServerConflicts(
				...
				.then(function (serverConflicts) {
					...
					return conflicts.concat(
						serverConflicts.filter(function (c) {
	...
	return this.dsItems
		...
		.then(function (data) {
			...
			data.forEach(function (av) {
				...
				transItem = items.find(function (item) {
...
Order.prototype.setFromDate = function (date, skipRead) {
	if (this.status != 'creating') {
		throw new api.ApiUnprocessableEntity('Cannot set order from date, status is ' + this.status);
	}
...
Order.prototype.clearFromDate = function (skipRead) {
	if (this.status != 'creating') {
		throw new api.ApiUnprocessableEntity('Cannot clear order from date, status is ' + this.status);
	}
...
Order.prototype.setDueDate = function (due, skipRead) {
	...
	if (this.status != 'creating' && this.status != 'open') {
		throw new api.ApiUnprocessableEntity('Cannot set order due date, status is ' + this.status);
	}
...
Order.prototype.clearDueDate = function (skipRead) {
	if (this.status != 'creating') {
		throw new api.ApiUnprocessableEntity('Cannot clear order due date, status is ' + this.status);
	}
...
Order.prototype.searchItems = function (params, useAvailabilies, onlyUnbooked, skipItems, listName) {
	return this._searchItems(
		params,
		listName != null ? listName : 'available',
		useAvailabilies,
		onlyUnbooked,
		skipItems || this.items
	);
};
...
Order.prototype.checkin = function (itemIds, location, skipRead, skipErrorHandling) {
	...
	return this._doApiCall({
		method: 'checkin',
		params: { items: itemIds, location: location },
		skipRead: skipRead,
	}).then(
		function (resp) {
			return resp;
...
Order.prototype.checkout = function (skipRead, skipErrorHandling) {
	var that = this;
	return this._doApiCall({ method: 'checkout', skipRead: skipRead }).then(
		function (resp) {
			return resp;
		},
		function (err) {
...
Order.prototype.undoCheckout = function (skipRead, skipErrorHandling) {
	var that = this;
	return this._doApiCall({ method: 'undoCheckout', skipRead: skipRead }).then(
		function (resp) {
			return resp;
		},
		function (err) {
...
Order.prototype.canExtend = function (due) {
	return Promise.resolve({ result: this.canExtendCheckout(due) });
};
...
Order.prototype._fromCommentsJson = function (data, options = {}) {
	...
	if (that.dsReservations && data.reservation && data.reservation.comments && data.reservation.comments.length > 0) {
		...
		return Base.prototype._fromCommentsJson
			.call(that, data.reservation, Object.assign(options, { ds: that.dsReservations, fromReservation: true }))
			.then(function () {
...
Order.prototype._fromAttachmentsJson = function (data, options = {}) {
	...
	if (
		that.dsReservations &&
		data.reservation &&
		data.reservation.attachments &&
		data.reservation.attachments.length > 0
	) {
		// Parse Reservation keyValues
		return Base.prototype._fromAttachmentsJson
			.call(that, data.reservation, Object.assign(options, { ds: that.dsReservations, fromReservation: true }))
			.then(function () {
